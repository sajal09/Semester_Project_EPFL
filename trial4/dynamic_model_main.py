...
model = RolandDynamicModelGRU(node_feat_dim=len(atom_mapping),
                             gnn_out=gnn_out,
...
if resume:
    wandb.init(project="semester_project_epfl", id="kr6ar8d4", resume='must')
...
for epoch in tqdm(range(start_epoch, epochs), desc="Training"):
    ...
    for batched_graph, target in train_dataloader:
        ...
        if (batched_graph.x.shape[0]/100) > graph_node_limit_train:
            print(f"Skipping graph in Train with {batched_graph.x.shape[0]} nodes")
            continue
    ...
    with torch.no_grad():
        for batched_graph, target in val_dataloader:
            ...
            if (batched_graph.x.shape[0]/100) > graph_node_limit_val:
                print(f"Skipping graph in Val with {batched_graph.x.shape[0]} nodes")
                continue
