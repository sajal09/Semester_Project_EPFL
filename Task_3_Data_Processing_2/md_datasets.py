...
from gnn_utils import build_frame_graph, get_atom_class, calc_atom_1hot, read_idx
...
class MDTrajDataset(Dataset):
    def __init__(self, k, distance_threshold, graph_type, processed_dir, datatype_ids, mdh5_file, T):
        ...
        self.qm_H5File = h5py.File("/work/lts2/users/sajal/data/QM.hdf5", 'r')
        self.ATOM_MAPPING = pickle.load(open("/work/lts2/users/sajal/data/task3/atom_classes.pickle", "rb"))
        self.affinity_df = pd.read_csv("/work/lts2/users/sajal/data/task3/all_dataset_merged.csv")
    ...
    def __getitem__(self, idx):
        ...
        one_hot_atom_features = calc_atom_1hot(torch.as_tensor(get_atom_class(group_in["atoms_type"], group_in["atoms_residue"], self.ATOM_MAPPING)), self.ATOM_MAPPING).float()
        one_hot_atom_features = one_hot_atom_features[selected_atom_indices, :]
        ...
        noHindices_lig = np.where(self.qm_H5File[protein_key]["atom_properties"]["atom_names"][()] != b"1")[0]
        charges_lig = torch.as_tensor(self.qm_H5File[protein_key]["atom_properties"]["atom_properties_values"][:, 7][noHindices_lig])
        charges_prot = torch.zeros(molecules_begin_atom_index[-1])
        charges = torch.cat((charges_prot, charges_lig)).unsqueeze(1)[selected_atom_indices, :]
        ...
        target = torch.tensor(self.affinity_df[self.affinity_df.PDB_code == protein_key]['-logKd/Ki'].item(), dtype=torch.float32) # (1,)
...
def create_dataset(data_type, k, distance_threshold, graph_type, folder_path, T):
    ...
    datatype_ids_file = os.path.join(data_type_parent_path, f'{data_type}_ids.txt')
