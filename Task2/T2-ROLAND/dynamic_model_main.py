...
from models import RolandDynamicModelGRUBatched
...
def collate(x):
    return Batch.from_data_list([i[0] for i in x]), [i[0].x.shape[0] for i in x], [int(i[0].x.shape[0]/T) for i in x], torch.cat([i[1] for i in x], axis = 0)
...
train_data_list = [train_dataset[i] for i in tqdm(range(len(train_dataset)), desc="Preloading train data")]
...
val_data_list = [val_dataset[i] for i in tqdm(range(len(val_dataset)), desc="Preloading val data")]
...
model = RolandDynamicModelGRUBatched(node_feat_dim=node_feat_dim, 
                             gnn_out=gnn_out,
...
checkpoint_folder = '/work/lts2/users/sajal/temp33/wandb/CheckpointDynamicModel'
...
for epoch in tqdm(range(start_epoch, epochs), desc="Training"):
    ...
    for batched_graph, block_lengths, block_lengths_dyn, target in train_dataloader:
        ...
        pred = model(batched_graph, block_lengths, block_lengths_dyn)     # (N, 2)
    ...
    if epoch % 4 == 0:
        model.eval()
        with torch.no_grad():
            for batched_graph, block_lengths, block_lengths_dyn, target in val_dataloader:
