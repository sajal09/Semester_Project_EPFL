...
class StaticDataset(Dataset):
    ...
    def __getitem__(self, idx, task1_features_dict):
        with h5py.File(self.mdh5_file, 'r') as md_H5File:
            ...
            dynamic_features = task1_features_dict[protein_key]
            combined_protein_atom_feats = np.hstack((protein_atom_feats, dynamic_features))  # (N, Combined Feature Dim)

            atoms_coords = md_H5File[protein_key]['atoms_coordinates_ref'][()]
            protein_coords = atoms_coords[:ligand_start] # (N, 3)
            protein_graph = build_frame_graph(protein_coords, combined_protein_atom_feats, self.k, self.distance_threshold, self.graph_type)
...
def create_static_model_dataset(data_type, k, distance_threshold, graph_type):
    ...
    mdh5_file = "/work/lts2/users/sajal/data/md_out.hdf5"#"/work/lts2/users/sajal/data/md_out.hdf5"
