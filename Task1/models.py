...
# ---------- T1-Static and T1-Static-Transfer ----------
...
class SpatialEGNN(nn.Module):
    def __init__(self, in_dim, hidden_dim, hidden_dim2, out_dim):
        super().__init__()
        self.conv1 = E_GCL(in_dim, hidden_dim, hidden_dim2, edges_in_d=1, act_fn=nn.ReLU(), 
                 residual=False, attention=False, normalize=True, coords_agg='mean', 
                 tanh=False, update_coords=False)
        self.norm1 = GraphNorm(hidden_dim)
        self.conv2 = E_GCL(hidden_dim, hidden_dim, hidden_dim2, edges_in_d=1, act_fn=nn.ReLU(), 
                 residual=True, attention=False, normalize=True, coords_agg='mean', 
                 tanh=False, update_coords=False)
        self.norm2 = GraphNorm(hidden_dim)
        self.conv3 = E_GCL(hidden_dim, hidden_dim, hidden_dim2, edges_in_d=1, act_fn=nn.ReLU(), 
                 residual=True, attention=False, normalize=True, coords_agg='mean', 
                 tanh=False, update_coords=False)
        self.norm3 = GraphNorm(hidden_dim)
        self.conv4 = E_GCL(hidden_dim, hidden_dim, hidden_dim2, edges_in_d=1, act_fn=nn.ReLU(), 
                 residual=True, attention=False, normalize=True, coords_agg='mean', 
                 tanh=False, update_coords=False)
        self.norm4 = GraphNorm(hidden_dim)
        self.conv5 = E_GCL(hidden_dim, out_dim, hidden_dim2, edges_in_d=1, act_fn=nn.ReLU(), 
                 residual=False, attention=False, normalize=True, coords_agg='mean', 
                 tanh=False, update_coords=False)
        self.norm5 = GraphNorm(out_dim)

    def forward(self, data):
        x, edge_index, edge_weight, pos = data.x, data.edge_index, data.edge_attr.unsqueeze(-1), data.pos
...
# ---------- T1-GCRN-EGNN ----------
class DynamicModelGRU_EGNN(nn.Module):
    def __init__(self, 
                node_feat_dim, 
                gnn_hidden, 
                gnn_hidden2,
        ...
        super().__init__()
        self.spatial_gnn = SpatialEGNN(node_feat_dim, gnn_hidden, gnn_hidden2, gnn_out)
        self.gru = nn.GRU(input_size=gnn_out, 
                          hidden_size=gru_hidden, 
                          num_layers=num_gru_layers, 
                          dropout=gru_dropout, 
...
# ---------- T1-ROLAND-GCN ----------
class RolandDynamicModelGRU(nn.Module):
    def __init__(self, 
                node_feat_dim,  
                gnn_out, 
                gru_hidden,
...
# ---------- Dynamic Model - S4 ----------
class DynamicModelS4(nn.Module):
    def __init__(self, 
                node_feat_dim, 
                gnn_hidden, 
                gnn_out, 
    ...
    def forward(self, graphs):
        ...
        # 2) Temporal GRU per atom
        H_stack = torch.swapaxes(H_stack, -1, -2)
        Z, _ = self.s4(H_stack)          # (N, gnn_out, T)
        Z, _ = self.s4_2(Z)              # (N, gnn_out, T)
        Z = torch.swapaxes(Z, -1, -2)    # (N, T, gnn_out)
