...
from models import RolandDynamicModelGRU_EGNN
...
model = RolandDynamicModelGRU_EGNN( node_feat_dim=node_dim,
...
resume = True
checkpoint_folder = '/home/chaurasi/semesterproject/temp15/wandb/CheckpointDynamicModel'

if resume:
    wandb.init(project="semester_project_epfl", id="m1cjj36a", resume='must')
...
for epoch in tqdm(range(start_epoch, epochs), desc="Training"):
    ...
    for batched_graph, target in train_dataloader:
        ...
        if (batched_graph.x.shape[0]/100) > graph_node_limit_train:
            print(f"Skipping train graph with {batched_graph.x.shape[0]} nodes")
            continue
        elif (batched_graph.x.shape[0]/100) > 8000:
            print(f"Large train graph with {batched_graph.x.shape[0]} nodes, clearing cache nonperiodically")
        ...
        with torch.no_grad():
            for batched_graph, target in val_dataloader:
                ...
                if (batched_graph.x.shape[0]/100) > graph_node_limit_val:
                    print(f"Skipping val graph with {batched_graph.x.shape[0]} nodes")
                    continue
                elif (batched_graph.x.shape[0]/100) > 10000:
                    print(f"Large val graph with {batched_graph.x.shape[0]} nodes, clearing cache nonperiodically")
                    torch.cuda.empty_cache()
                    gc.collect()
