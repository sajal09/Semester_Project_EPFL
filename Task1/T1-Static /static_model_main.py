...
train_dataset = create_static_model_dataset('train', k, distance_threshold, graph_type)
...
resume = True
checkpoint_folder = '/home/chaurasi/semesterproject/temp2/wandb/CheckpointStaticModel'

if resume:
    wandb.init(project="semester_project_epfl", id="8yhnkyo6", resume='must')
...
for epoch in tqdm(range(start_epoch, epochs), desc="Training"):
    ...
    for batched_graph, target in train_dataloader:
        ...
        if (batched_graph.x.shape[0]) > graph_node_limit:
            continue
        elif (batched_graph.x.shape[0]) > 2000:
