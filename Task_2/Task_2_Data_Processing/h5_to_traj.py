...
# 6peg => Some problem with its restart file, skip it for now.
...
def create_traj_files():
    # Read the MD.hdf5 file
    f_path = "/work/lts2/users/sajal/data/MD.hdf5"
    f = h5py.File(f_path, 'r')

    # Restart files path
    rstpath = '/scratch/izar/chaurasi/restart_files/parameter_restart_files_MD'

    #Output folder path
    output_folder = '/scratch/izar/chaurasi/traj_files'

    # For each structure, create the traj .top and .nc files
    for struct in f.keys():
        print("Processing structure: ", struct)
        traj = open_restart_file(struct.lower(), rstpath)
        h5_coordinates = get_entries(struct, f)
        # This mask strippes all Waters and Ions. 
        mask = "!:WAT,Na+,Cl-"
        new_traj = create_new_traj(traj[mask], h5_coordinates)
        pt.write_traj(os.path.join(output_folder, struct + ".nc"), new_traj, overwrite=True)
        create_topology(os.path.join(output_folder, struct + ".top"),  os.path.join(rstpath,struct.lower(),"production.top"), mask)
