...
def aligned_frames(h5_entries):
    """
    Calculates the aligned coordinates of the atoms for each frame.
    Args:
    h5_entries (dict): Dictionary of h5 entries
    ...
    ref = h5_entries["trajectory_coordinates"][0]
    NAtom = len(ref)
    aligned_frames_mat = np.zeros((100, NAtom, 3))
    for ind in range(100):
        aligned = align_frame_to_ref(h5_entries, ind, ref)
        aligned_frames_mat[ind, :, :] = aligned
    return aligned_frames_mat
    ...
    for struct in structs[args.begin:args.end]:
        ...
        if not h5_entries["atoms_type"] == None: 
            ...
            if args.Adaptability:
                ...
                preprocessing_entries["trajectory_coordinates"] = aligned_frames(preprocessing_entries)
                preprocessing_done = True
